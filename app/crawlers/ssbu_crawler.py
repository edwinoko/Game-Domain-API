...
def create_local_data_structure(title, url):
    ...
    if not path.exists(cwd+'/data/smash_character_data/'):
        makedirs(cwd+'/data/smash_character_data')
    #creating a file    
    with open(cwd+'/data/smash_character_data/'+title+'.json','w+') as char_file:
